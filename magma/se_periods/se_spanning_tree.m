...
// Computes a spanning tree between Points
	...
	if S`IntegrationType eq "GC" and T`IntPar-1 lt 10^-3 then
		S`IntegrationType := "DE";
		vprint SE,1 : "Changed integration type to",S`IntegrationType,"due to bad integration parameter:",T`IntPar;
		SpanningTree(S);
	else
		S`SpanningTree := T;
	end if;	
