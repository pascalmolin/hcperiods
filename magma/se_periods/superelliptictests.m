...
{ Tests if period matrices of superelliptic curves are equivalent }
	...
	time PM2 := RS_SECurve(s:Prec:=Prec);
	print "Searching symplectic transformation...";
	bool, S := IsIsomorphicSmallPeriodMatrices(PM1,PM2`SmallPeriodMatrix);
	return bool, S;
end intrinsic;
...
intrinsic RS_SEAJMTest_Inf3( SEC::SECurve ) -> RngIntElt
{ Computes AJM for a principal zero divisor }
	"#####################  TEST ####################";
	delta := SEC`Degree[3];
	assert delta gt 1;
	...
	// For each points at infinity compute AJM of D = [ r - zeta^k ]
	f := Ct!SEC`UnivariatePolynomial;
	//f *:= 1/(LeadingCoefficient(f));
	g_t := f - LeadingTerm(f);
	print "g_t:",g_t;
	if Degree(g_t) eq n-1 then
		print "Good case!";
	else
		print "Bad case!";
		//assert false;
	...
	Rts_g_t := Roots(g_t);
	print "SEC`BranchPoints:",SEC`BranchPoints;
	cf := -LeadingCoefficient(f); print "cf:",cf;
	ord := n-Degree(g_t);
	print "Ord:",ord;
	print "Rts_g_t:",Rts_g_t;
	Points := < >; Coeffs := [];
	for j in [1..Degree(g_t)] do
		t_j := Rts_g_t[j][1];
		if Abs(t_j) gt SEC`Error then
			Y := RS_PrincipalBranch(SEC,t_j:Fiber); print "Y:",Y;
			//print "x_j:",x_j;
			x_j := t_j;
			for k in [1..m] do
				print "Y[k]^M:",Y[k]^M;
				print "x_j^N:",x_j^N;
				if Abs(Y[k]^M - (cf^(1/n)*x_j)^N) lt SEC`Error then 
					Append(~Points,[x_j,Y[k]]);
	...
	if #Coeffs ne 0 then
		...
		V := Matrix(SEC`RealField,2*SEC`Genus,1,[]);
	end if;
	
	print "V:",V;
	print "(1/ord)*V:",(1/ord)*V;
	print "SEC`AJM_InftyPoints[1]:",SEC`AJM_InftyPoints[1];
	Diff := (1/ord)*V-SEC`AJM_InftyPoints[1];
	print "Diff:",Diff;
	print "ord * Diff:",ord*Diff;
	return 0;
end intrinsic;



procedure RS_TestForARB( DataSet )
// Test periods
	Path := "/home/neurohr/superelliptic/magma/testperiods.txt";
	SetOutputFile(Path:Overwrite:=true);
	for Data in DataSet do
		Points := Data[1];
		for m in Data[2] do
			Prec := Precision(Universe(Points));
			AB := RS_SEPeriodMatrix( Points, m : Prec := Prec, Small := false );
			g := NumberOfRows(AB);
			assert g gt 1;
			a11 := AB[1][1];
			print Re(a11); print Im(a11);
			//Write(path,String(a11));
			a12 := AB[1][2];
			print Re(a12); print Im(a12);
			a21 := AB[2][1];
			print Re(a21); print Im(a21);
			a22 := AB[2][2];
			print Re(a22); print Im(a22);
			b11 := AB[1][g+1];
			print Re(b11); print Im(b11);
			b12 := AB[1][g+2];
			print Re(b12); print Im(b12);
			b21 := AB[2][g+1];
			print Re(b21); print Im(b21);
			b22 := AB[2][g+2];
			print Re(b22); print Im(b22);
		end for;
	end for;
	UnsetOutputFile();	
end procedure;
