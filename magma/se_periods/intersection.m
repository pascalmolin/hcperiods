...
import "superelliptic.m": RS_ImSgn, RS_NRootAC, RS_MakeCCVectors, RS_SortByRealPart;
...
intrinsic RS_SEIntersection( Edge1::Tup, Edge2::Tup, Points::[FldComElt], m::RngIntElt, Zetas::SeqEnum[FldComElt] ) -> Tup
{ - }
	...
	CCV1, up1 := RS_MakeCCVectors(Edge1[1],Edge1[2],Points);
	CCV2, up2 := RS_MakeCCVectors(Edge2[1],Edge2[2],Points);
	...
	if Edge1[1] eq Edge2[1] then
		//"################################ Case1: a = c ################################";
		AR1 := RS_NRootAC(-1,CCV1,Zetas,up1,n-2,m);
		AR2 := RS_NRootAC(-1,CCV2,Zetas,up2,n-2,m);
		Val_ab := C_ab * AR1;
		Val_cd := C_cd * AR2;		
		Val := Val_cd/Val_ab;
		k_ := ((1/(2*C_Pi)) * ( phi + m * Arg(Val) ));
		k := Round(k_);
