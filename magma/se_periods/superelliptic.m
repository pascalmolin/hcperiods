...
import "spanning_tree.m":TreeData;
...
function RS_SortByRealPart(CCV)
	OrdCCV := []; up := 0;
	for z in CCV do
		if Re(z) le 0 then
			Append(~OrdCCV,z);	
...
function RS_MakeCCVectors( i1, i2, Points )
	...
	if Type(i2) eq RngIntElt then
		...
		CCV, up := RS_SortByRealPart([ (2*x-b-a)/(b-a) : x in Pts ]);
		bma := b-a;
		Append(~CCV,bma/2);
		Append(~CCV,(b+a)/bma);
		...
		CCV, up := RS_SortByRealPart([ (2*x-p-a)/(p-a) : x in Pts ]);
		pma := p-a;
		Append(~CCV,pma/2);
		Append(~CCV,(p+a)/pma);
	else
...
// Computes a minimal spanning tree w.r.t holomorphicy
	...
	//MST_Edges := [<1,5>,<2,5>,<3,5>,<4,5>,<6,5>,<7,5>]; Tau := -0.2;
...
function RS_NRootAC(x,CCV,Zetas,up,d,m:long:=false)
...
function RS_SEIntegrate( Edge,Points,DFF,Abscissas,Weights,StepLength,n,m,Zetas,lc_m )
	...
	CCV, up := RS_MakeCCVectors(Edge[1],Edge[2],Points);
	...
	z := 1/RS_NRootAC(0,CCV,Zetas,up,n-2,m);
	...
	for t in [1..#Abscissas] do
		...
		z1 := 1/RS_NRootAC(x,CCV,Zetas,up,n-2,m);
		z2 := 1/RS_NRootAC(-x,CCV,Zetas,up,n-2,m);
		x1 := x + Fact2;
		x2 := -x + Fact2;
		Enum1 := x1^DFF[1][1][1];
		Enum2 := x2^DFF[1][1][1];
		...
		// Weights
		w2_2m_inv := Exp( (2/m) * -Log(Weights[t][2]));
		dx := Weights[t][1] * Weights[t][2]^2 * w2_2m_inv^DFF[1][1][2];
		dx_ := dx;
	...
	for j in [2..#DFF] do
		...
		Diff := DFF[j][1][2] - DFF[j-1][1][2];
		for l in [1..Diff] do
			Factor2 *:= Fact1_nm_inv;
		end for;
		Factor21 := Factor2;
		Ct +:= 1;
		Pow := -((up + 1) mod 2)*DFF[j][1][2] mod (2*m) + 1;
		Integrals0[Ct] *:= lc_m^DFF[j][1][2] * Zetas[Pow] * Factor1 * Factor21;
		Append(~ElementaryIntegrals,Integrals0[Ct]);
		Pow := -2*DFF[j][1][2] mod (2*m) + 1;
		Integrals0[Ct] *:= (1 - Zetas[Pow]);
		NextIntegrals := [ Integrals0[Ct] ];
		for l in [1..m-2] do
			Pow := -2*l*DFF[j][1][2] mod (2*m) + 1;
			Append(~NextIntegrals,Zetas[Pow]*Integrals0[Ct]);
		end for; 
		Append(~Integrals,NextIntegrals);
		for k in [2..#DFF[j]] do
			Factor21 *:= Fact1_nm_inv;
			Ct +:= 1;
			Pow := -((up + 1) mod 2)*DFF[j][k][2] mod (2*m) + 1;
			Integrals0[Ct] *:= lc_m^DFF[j][k][2] * Zetas[Pow] * Factor1 * Factor21;
			Append(~ElementaryIntegrals,Integrals0[Ct]);
			Pow := -2*DFF[j][k][2] mod (2*m) + 1;
			Integrals0[Ct] *:= (1 - Zetas[Pow]);
			NextIntegrals := [ Integrals0[Ct] ];
			for l in [1..m-2] do
				Pow := -2*l*DFF[j][k][2] mod (2*m) + 1;
				Append(~NextIntegrals,Zetas[Pow]*Integrals0[Ct]);
			end for; 
			Append(~Integrals,NextIntegrals);
		end for;
...
function RS_SEIntegrate2( Edge,Points,DFF,Abscissas,Weights,StepLength,n,m,Zetas,lc_m )
	...
	// Initiate on x = 0, dx = 1
	z := RS_NRootAC(0,CCV,Zetas,up,n-2,m);
	for j in [1..g] do
		w := DFF[j];
		Denom := z^w[2];
	...
	for t in [1..#Abscissas] do
		...
		// Analytic continuation
		z1 := RS_NRootAC(x,CCV,Zetas,up,n-2,m);
		z2 := RS_NRootAC(-x,CCV,Zetas,up,n-2,m);
		...
		for j in [1..g] do
			...
			Denom1 := z1^w[2];
			Denom2 := z2^w[2];
			dx := Weights[t][1] * Weights[t][2]^(2-(2*w[2]/m));
	...
	for j in [1..g] do
		...
		Pow := -((up + 1) mod 2)*w[2] mod (2*m) + 1;
		Integrals0[j] *:= lc_m^w[2] * Zetas[Pow] * StepLength * Factor;
		Append(~ElementaryIntegrals, Integrals0[j]);
		Integrals0[j] *:= ( 1 - Zetas[3]^-w[2]);
		Append(~Integrals,[ Zetas[3]^(-l*w[2]) * Integrals0[j] : l in [0..m-2]]);
	end for;
	return Integrals, ElementaryIntegrals;
end function;


function RS_SEIntegrate3( Edge,Points,DFF,Abscissas,Weights,StepLength,n,m,Zetas,lc_m )
	...
	// Initiate on x = 0, dx = 1
	y1 := 1/RS_NRootAC(0,CCV,Zetas,up,n-2,m); // 1/y(0)
	wy1 := y1;
	iy := 1;
	for j in [1..g] do
		w := DFF[j];
	...
	for t in [1..#Abscissas] do
		x := Abscissas[t];
		y1 := 1/RS_NRootAC(x,CCV,Zetas,up,n-2,m); // 1/y(x)
		y2 := 1/RS_NRootAC(-x,CCV,Zetas,up,n-2,m); // 1/y(-x)
		w2N := Exp( -(2/m) * Log(Weights[t][2])); // cosh^-(2/N)
		y1 *:= w2N;
		y2 *:= w2N; 
...
{ Wrapper function }
	...
	if Prec lt 20 then
		if IsExact(BaseRing(Parent(f))) then
			Prec := 20;
		else
			Prec := Precision(BaseRing(f));
		end if;
	end if;
	//Extra := Ceiling(Log(10,Binomial(Degree(f,1),Round(Degree(f,1)/2)))); print "Extra:",Extra;
	Extra := 5;
	CC<i> := ComplexField(Prec+Extra); CCx<x> := PolynomialRing(CC);
	// Branch Points
	f_x := CCx!UnivariatePolynomial(Evaluate(f,[Parent(f).1,0]));
	Points :=  RS_Roots(f_x);
	Points := [ CC!P : P in Points ];
	// Leading coefficient
	lc_x := -LeadingCoefficient(f_x);
	return RS_SEPeriodMatrix( Points, Degree(f,2) : Prec := Prec, LeadingCoeff := lc_x, Small:=Small );
end intrinsic;


intrinsic RS_SEPeriodMatrix( Points::SeqEnum[FldComElt], m::RngIntElt : LeadingCoeff := 1, Prec := -1, Small := true ) -> Mtrx
{ Computes period matrices associated to the superelliptic curve defined by f to precision Prec }

	// Precision
	CompPrec := Precision(Universe(Points));
	vprint SE,1 : "Precision of points:",CompPrec;

	if Prec lt 20 then
		Prec := 20;
	end if;
	Prec := Min([Prec,CompPrec]);
	vprint SE,1 : "Precision for output:",Prec;

	// Low precision branch points
	LowPrecPoints := [ C_20!Pt : Pt in Points ];
	vprint SE,2 : "Branch points:",LowPrecPoints;
	
	// Error
	Err := 10^-(Prec+1);

	// Degrees and genus
	n := #Points; g := Round((1/2) * ((m-1)*(n-1) - Gcd(m,n) + 1));
	...
	vprint SE,2 : "Leading coefficient:",C_20!LeadingCoeff;	
	lc_m := Exp( -(1/m) * Log(LeadingCoeff));
	print "lc_m:",lc_m;
	// Maximal spanning tree
	vprint SE,1 : "Constructing spanning tree...";
	t := Cputime();
	if n ge 15 then
		vprint SE,1 : "using minimal euclidean distance weights...";
		MST_Edges, MST_Tau := RS_SEMST2(LowPrecPoints:ED:=true);
		//SpTree := RS_SpanningTree(LowPrecPoints:IntType := IntType,ED);
	else
		vprint SE,1 : "using maximum holomorphic weights...";
		MST_Edges, MST_Tau := RS_SEMST2(LowPrecPoints:ED:=false);
		//MST_Edges, MST_Tau := RS_SEMST(Points);
		//SpTree := RS_SpanningTree(LowPrecPoints:IntType := IntType);
	end if;
	// Compute TreeData
	//TreeData(~SpTree,Points);


	vprint SE,1 : Cputime(t);
	
	vprint SE,2 : "MST_Edges:",MST_Edges;
	//vprint SE,2 : "#MST_Edges:",SpTree`Len;
	vprint SE,1 : "MST_Tau:",C_20!MST_Tau;
	//MST_Edges := SpTree`Edges;
	//MST_Tau := SpTree`IntPar;
	// Computation of periods
	Integrals := [];
	CurveData := <n,g,lc_m,CC>;
	if m eq 2 then
		vprint SE,1 : "Using Gauss-Chebychev integration...";
		vprint SE,1 : "Integrating...";
		t := Cputime();
		Integrals := GC_Integrals_Tree(SpTree,CurveData);
		vprint SE,1 : Cputime(t);
	else
		// Holomorphic differentials
		vprint SE,1 : "Computing holomorphic differentials...";
		//DFF := RS_SEDifferentials(m,n:Int3:=false);
		DFF := RS_SEDifferentials(m,n:Int3);
		...
		// Integrals := DE_Intergral_Tree(SpTree,CurveData);


		for k in [1..n-1] do
			vprint SE,2 : "Integrating edge",MST_Edges[k];
			...
			Append(~Integrals,I);
			vprint SE,3 : "gives (g x (m-1))-periods:",I;
		end for;
		vprint SE,1 : Cputime(t);
	end if;
	...
	//vprint SE,3: "Integrals (Periods):",PM_;
	...
	vprint SE,1 : Cputime(t);
	vprint SE,2: "Intersection matrix:",RealIntMatrix;
	vprint SE,3: "Rank(IntersectionMatrix):",Rank(RealIntMatrix);
	...
	vprint SE,3: "ST:",ST;
	vprint SE,3: "CF:",CF;
	//vprint SE,1 : "Maximal entry in ST:",Max( [Abs(z): z in Eltseq(ST) ]);
	...
	if Small eq false then
		...
		vprint SE,1 : "Big Period Matrix:";
		return PM;
	end if;

	vprint SE,1 : "Inverting A-periods...";
	t := Cputime();
	PM_AInv := PM_A^(-1);
	vprint SE,1 : Cputime(t);


	vprint SE,1 : "Multiplying A^(-1)*B...";
	t := Cputime();
	PM := PM_AInv * PM_B;
	vprint SE,1 : Cputime(t);
	...
	vprint SE,1 : "Small period matrix Tau = A^(-1)B :";
	return PM;
...
// Compute the bound M1 on the integrand
	B1 := 1/(&*[ RS_Dist_1(CCV[k]) : k in [1..#CCV-2] ])^(1/N);
	//print "B1:",B1;
	if B1 gt 1 then
		return B1^(N-1);
	else
...
// Compute the bound M2 on the integrand
	B2 := 1/(&*[ RS_Dist_thsh(CCV[k],Tau) : k in [1..#CCV-2] ])^(1/N);
	//print "B2:",B2;
	if B2 gt 1 then
		return B2^(N-1);
	else
