...
The rigorous numerical evaluation of \eqref{eq:elem_num_int} is
adressed in Section~\ref{sec:numerical_integration}: for any edge $(a,b)$,
Theorem~\ref{th:de} and \ref{th:gc} provide
an explicit scheme allowing to attain any prescribed precision.
  ...
  From an a priori analysis of all numerical integrals $I_{a,b}$ along
  the interval $[a,b]$, we choose an optimal set of edges forming a spanning tree as follows:
  \begin{itemize}
      \item
   Consider the complete graph on the set of finite branch points $G = (X,E')$,
   i.e.\ $E' = \{  (a,b)  \mid  a,b \in X \}$.
      \item
   ...
   that results in a spanning tree $T = (X,E)$, where $E \subset E'$ contains the $n-1$ best edges
  ...
  \end{itemize}
   ...
   (this notion is made explicit in Section \ref{sec:numerical_integration}).
