...
\begin{document}
  ...
  \end{thm}
   ...
   the number of integration points defined in Proposition \ref{prop:Ngc}.
   \begin{itemize}
       \item 
   ...
   Let $K$ be the number of brach points $u_k$ such that $r_k=r_0$ and 
   \[ M_0 = (\prod_{r_k = r_0} D_k\prod_{r_k>r_0}d_k)^{-j/m}, \]
   then the integrand is bounded on $ε_{r_0-η}$ by
   \[ M(r_0-η) = M_0 η^{-\frac{Kj}m} (1+O(η)). \]
   Plugging this into \eqref{eq:Ngc} and letting
   $A = D+\log(2πM_0)$ and $B=jK/m$, the number of integration points
   satisfies
   \[
       2N = \frac{A - B \log(η) }{r_0-η}(1+O(η)).
   \]

   The main term is minimized for $η$ satisfying
   $η(A+B-B\log(η))=Br_0$,
   we can take a first order approximation
   \[ η = \frac{Br_0}{A+B(1+\log\frac{A+B}{Br_0})}. \]
 
   \subsubsection{Double-exponential case}

   We use the parametrization
   $\partial Z_r = \set{ z = \tanh(λ\sinh(t+ir), t\in\R }$.
   so that the distance from a branch point $u_k$ to $Z_r$
   can be computed applying Newton method to the scalar
   product function
   ...
   the parameter $r<r_0$ is chosen, we use ball arithmetic to compute a rigorous
