  ...
  Theorem \ref{m-thm:complexity_integrals}.
   ...
   \begin{itemize}
     \item[(i)] For each finite point $P \in \caff$ we can compute $\int_{P_0}^P \bar\w$ to precision $D$ using 
      $\ctot(D)$ operations. 
     \item[(ii)] For each infinite point $P_{\infty} \in \cu$ we can compute a representative of $\int_{P_0}^{P_{\infty}} \bar\w \mod \lambda$ to precision $D$ using
      \begin{itemize}
      ...
      \end{itemize}
      \item[(iii)] Reducing a vector $v \in \C^g$ modulo $\Lambda$ can be done using $O(g^{2.8})$ multiplications.
    \end{itemize}
  \end{thm}
   \begin{proof}
    \begin{itemize}
   ...
   of integrals \eqref{m-eq:periods}. According to \S~\ref{sec:comp_elem},
   ...
   and the latter when the $g>n$.
    ...
    Following our definition \eqref{eq:def_yab}, computing $\ytab(u_k)$ involves
