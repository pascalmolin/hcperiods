...
\begin{document}
  ...
  \section{Computational aspects}\label{sec:comp_asp}
   ...
   \begin{equation*}
       I_{a,b} \approx \sum_{k=1}^N w_k\frac{v_k^{i-1}}{y_k^j},
   \end{equation*}
   where $N = N(D)$ is the number of integration points, $w_k,v_k$ are integration weights and points,
   and $y_k=\ytab(v_k)$.

   We proceed as follows:
   \begin{itemize}
   \item for each $k$, we evaluate the absissa and weight $v_k,w_k$ using
   ...
   \item we compute $y_k=\ytab(v_k)$ using $n-2$ multiplications and one $m$-th root,
       as shown in \S \ref{m-subsec:computing_roots} below;
   \item starting from $\frac{w_k}{y_k}$, we evaluate all $g$ terms $w_k\frac{v_k^{i-1}}{y_k^j}$
       each time either multiplying by $v_k$ or by $\frac{1}{y_k}$, and add each to the corresponding
       integral.
   \end{itemize}
  ...
  \end{thm}
   ...
   The distance $d_k=\dist(u_k,ε_r)$ from a point $u_k \in U^+ \cup U^-$
   ...
   Let $K$ be the number of points $u_k 
   \in U^+ \cup U^-$ such that $r_k=r_0$ and
   ...
   the distance from a point $u_k \in U^+ \cup U^-$ to $Z_r$ by Newton's method
   as before.
   ...
   for each integration point $v_k\in]-1,1[$ one first evaluates the $y$-value
   $y_k=\ytab(v_k)$, then adds the contributions $w_k\frac{v_k^i}{y_k^j}$ to
   ...
   and the latter when the $g \gg n$.
    ...
    Following our definition \eqref{m-eq:def_yab}, computing $\ytab(v_k)$ involves
  ...
  Lemma \ref{lemma:wind_numb} can easily be turned into an algorithm that computes $q(u)$.
   ...
   $v_k$, which is usually done $g-m-1$ times for each of
   ...
   $\tilde{v}_k=v_k+\frac{b+a}{b-a}$. However, $\tilde{v}_k$ is a complex number while $v_k$
   is real, so computing with $v_k$ saves a factor almost $2$ on this aspect.
