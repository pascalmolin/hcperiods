   ...
   \subsubsection{Maximal spanning tree}
    ...
    \begin{equation}
     r_e =  \min_{c \in X \setminus \{ a,b\}} \begin{cases}
            ...
            \end{cases}
    \end{equation}
    Although this can be done in low precision, computing $r_e$ for all $(n-1)(n-2)/2$ edges of the complete graph needs
    \begin{equation} O(n^3)
     \begin{cases}
   \text{ multiplications, if $m=2$ or}\\
    \text{ evaluations of transcendental functions, if $m>2$}.
    \end{cases}
    \end{equation}
    ...
    between the end points of an edge as capacity, i.e. $r_e = |b-a|$, which  reduces the complexity to $O(n^2)$ multiplications.
