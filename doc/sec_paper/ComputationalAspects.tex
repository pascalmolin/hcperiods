...
\begin{document}
  ...
  Lemma \ref{lemma:wind_numb} can easily be turned into an algorithm that computes $q(u)$.
   ...
   \subsubsection{Near-optimal tree}
    ...
    if some branch points tend to cluster while other are far away. A simple
    ...
    relative distances of the singularities: in the case of double-exponential
    integration, writing
    $\int_0^{1000}=\int_{0}^{6}+\int_6^{1000}$ gives two integrals with
    $r=0.48$ each. Splitting further at $2$ and $33$ gives $r=0.63$.

    Another option with double exponential integration is to shift
    the integration path in the change of variable
    as explained in \cite{Molin2010}.
