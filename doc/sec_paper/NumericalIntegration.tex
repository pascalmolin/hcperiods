...
\begin{document}
  ...
  \section{Numerical integration}\label{sec:num_int}
...
\begin{equation}
    ...
    I_{a,b}(i,j) = \int_{-1}^1\frac{u^{i-1}\du}{(1-u^2)^{frac jm}\ft_{a,b}(u)^j}
\end{equation}
...
u=\tanh(λ\sinh(t)),
\end{equation}
the integral \eqref{eq:elemn_num_int} becomes
\begin{equation}
...
polynomial $Q(u)=\prod(u-u_k)$, so that the error can be controlled
as follows
\begin{prop}
    Let $r>1$ be such that $2r<\abs{u_i-1}+\abs{u_i+1}$ for all
    roots $u_i$ of $Q$, and denote by
    \begin{equation}
        d_r(u_i) = \inf_{z\in ε_r}\abs{z-u_i}
    \end{equation}
    the distance from $u_i$ to the ellipse $ε_r$.
    Then writing
    \begin{equation}
    ...
    \begin{equation}
        \abs{I_{a,b}(i,j)
        - \frac{π}n\sum_{k=1}^n \frac{u_k^(i-1)}{\fab(u_k)}}
            \leq e^{-D}
    \end{equation}
    for all
    \begin{equation}
