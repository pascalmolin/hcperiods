...
This representation has the following advantages:
 ...
 \subsubsection{Non-coprime degrees}\label{subsec:ajm_inf_ncop}
  ...
  Hence, we can describe the points in $\mathcal{P} \subset \cu$ via
   ...
   \end{equation*}
      where the $t_i^{(s)}$ are the zeros of $h(t) = \prod_{k=1}^n (1-x_k\zeta_{\delta}^{s\nu}t^M) - 1 \in \C[t]$ and
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      \equiv  -\AJ \left( \left[\sum_{i = 1}^{d-1} \Phi(\zeta_{\delta}^s,t_i^{(s)}) - N \sum_{Q \in\text{pr}_x^{-1}(0)} Q  \right] \right)
       \mod \Lambda.
    \end{align*}
    ...
    \begin{align*}
    -\AJ( [\sum_{i = 1}^{d-1} \Phi(\zeta_{\delta}^s,t_i^{(s)}) - N \sum_{Q \in\text{pr}_x^{-1}(0)} Q ] ) \equiv
    ...
    The corresponding principal divisor is given by
     \begin{equation*}
      ...
      \Phi^{-1}(x_k,0)- N E_2',
   \end{equation*}
      where the $t_i^{(s)}$ are the zeros of $h(t) = \prod_{k=1}^n (1-x_k(t+\zeta_{\delta}^{(s)})^{\nu}t^M) - 1 \in \C[t]$, $d = \deg(h)$ and
   ...
   \begin{proof}
    ...
    \begin{align*}
     \AJ( [D_{\infty}^{(s)}] ) \equiv & \AJ([D_{\infty}^{(s)} - D_2]) \\
      ...
      (x_k,0) - N \sum_{Q \in\text{pr}_x^{-1}(0)} Q ] )
       \mod \Lambda.
    \end{align*}
   Choosing the points
