...
%        $\cu \rightarrow \P^1_K$ of degree $m > 1$ such that $\text{char}(K)$ and $m$ are coprime.
  ...
  principal branch of the $m$-th root $\sqrt[m]z$ defined by
  \begin{equation}
      \label{eq:principal_mth_root}
      -\frac{π}m<\arg(\sqrt[m]z)\leq\frac{π}m
  \end{equation}
  ...
  that by taking the cut towards $+\infty$ for each point $x_k$ with positive real part, writing
  \begin{equation*}
  ...
  \begin{figure}[H]
      \begin{center}
          ...
          \subfloat[$y_{a,b}$]{ \includegraphics[width=.3\linewidth,page=3]{images/branch_cuts.pdf}\label{fig:mth_root_signed} }
      \end{center}
  ...
  \end{equation*}
  where points in $U^+$ (resp. $U_-$) have strictly positive (resp. negative) real part.

  Then the product
  \begin{equation}
      \label{eq:fab}
      \ft_{a,b}(u) = \prod_{u_k\in U^-} \sqrt[m]{u-u_k}\prod_{u_k\in U^+}\sqrt[m]{u_k-u}
  \end{equation}
  is holomorphic on a neighborhood $ε_{a,b}$ of $[-1,1]$ which we can take as
  an ellipse\footnote{we will exhibit such a neighborhood in section \ref{sec:gc_ellipse}}
  ...
  \begin{equation}
      \label{def:yab}
      \yab(x) =   C_{a,b} \ft_{a,b}( u_{a,b}(x) ) \sqrt[m]{1 - u_{a,b}(x)^2}
  \end{equation}
  where the constant
  \begin{equation}
  ...
  More precisely, let $V_{a,b} = u_{a,b}^{-1}(ε_{a,b}\cap \overline U)$,
  ...
  \begin{prop}
      ...
      $\fab$ \eqref{eq:fab} and $y_{a,b}$ \eqref{eq:yab}
      satisfy
     \begin{itemize}
         \item $\ft_{a,b}$ is holomorphic and does not vanish on $ε_{a,b}$
