...
\begin{document}
  ...
  which we will briefly discuss here. For an analytic multi-valued function we define a \emph{branch cut}
  as a curve in the complex plane across which this function is discontinous. 
  We were able to make good use of this concept since our superelliptic curves
  inherit their branch cuts from the complex $m$-th root, which can be explicitly controlled.
  Before specifying a branch the $m$-th root is the multi-valued function $y^m = x$.
  ...
  \end{equation*}
  We obtain a Riemann surface from the $m$-th root by glueing together $m$ copies of the complex plane (the \emph{sheets})
  along the branch cut $]\!-\infty,0]$. The branch points of this surface
  are at $x = 0,\infty$, and these are totally ramified. In
  ...
  %to continue $y$ as an analytic function following a path in the $x$-plane. This 
  ...
  %Similar to the complex $m$-th root, 
  Since the branch cuts at $x_k \in X$ are inherited from the complex $m$-th root 
  we can assume that crossing the branch cut in positive direction corresponds to multiplication by $\zeta$ on the Riemann surface. We
