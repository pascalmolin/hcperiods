...
%           +\delta_{a=c}\frac{1}2
	    s(x) = \frac{1}{2\pi}\left( \rho + m \cdot \arg \left( \frac{(d-c)^{\frac{\deg(f)}{m}}\ytcd(x)}{(b-a)^{\frac{\deg(f)}{m}}\ytab(x)} \right) \right) + \frac{1}{2}(U_{c,d}^+ - U_{a,b}^+
	    + \delta_{b=c})
      \end{equation}
     and
      \begin{equation}
 ...
 \todo (CN) You have to beware of the branch cut of the $\arg$-function!
