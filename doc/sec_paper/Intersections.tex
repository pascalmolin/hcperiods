...
\begin{document}
 ...
 such that $\pr(\cyabk) \cap \pr(\cycdl) = \{ \xt \}$ for some $\xt \in V$. Consequently, on $\cu$ they can at most intersect at the
...
\end{figure}
 ...
 For for $\varphi = \arg\left(\frac{b-a}{d-c}\right) > 0$, the non trivial cases, i.e. $s(\xt) + l - k \in \{1,0,-1\}$, are shown in Figure \ref{m-fig:int_a=c} 
...
\end{lemma}
 ...
 Starting from equation \eqref{m-eq:sxt}, for all $x \in \C \setminus X$ we have
  ...
  \end{align*}
 Assume we are in case $(ii)$, i.e. $b = c$ and denote $\varphi_0 = \arg \left( \frac{b-a}{d-c} \right)$. Then, we can parametrize all points $\xt \ne b$ on 
 ...
 \end{align*}
thus proving (1).
  In the cases $(iii)$ and $(iv)$ we have $a = c$ and denote $\varphi =  \arg \left( \frac{b-a}{d-c} \right)$. If $\phi > 0$, then we can parametrize all points on the inner
 ...
 \end{align*}
 As before we can let $\xt$ tend towards $a$ and compute the shifting function at $\xt$ as
 \begin{align*}
 ...
 \end{align*}
 Since the case $\phi < 0$ follows analogously, the proof of (2) is completed.
\end{proof} 

\begin{rmk}
  The intersection numbers given by Theorem \ref{m-thm:intsec_numb} are independent of the choice of $\xt$.
  If we were to choose $\xt$ on the lower (or outer) bisectrix, then $[\xt,c[$ would be contained in a different connected component of $V$. The value of
  $s(\xt)$ changes, but the parametrization of the bisectrix and the corresponding arguments will change accordingly.
\end{rmk}
