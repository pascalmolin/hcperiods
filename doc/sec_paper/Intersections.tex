...
\begin{document}
  ...
  such that $c$ is either $a$ or $b$. Moreover, we may discard the case $a=c$ and $c=d$.
...
\end{figure}
 ...
 Instead, we choose a point $\xt \in \C \setminus X$ on the bisectrix of
  ...
  \end{equation}
    \begin{figure}[H]
      \begin{center}
   \scalebox{.7}{\input{images/set_v_both.tex}}
      \end{center}
     \vspace{-1cm}
 ...
 For $\varphi = \arg\left(\frac{b-a}{d-a}\right) > 0$, the non trivial cases, i.e. $s(\xt) + l - k \in \{-1,0,1\}$, are shown in Figure \ref{m-fig:int_a=c}
  ...
  fall back to case (iii) by looking at the negative of the intersection of the swapped cycles
...
\begin{lemma}\label{lemma:sxt=sx}
  ...
  \begin{itemize}
   \item[(1)] $s(\xt) \equiv s_b \bmod m$ in case {\upshape{(ii)}},
    \item[(2)]  $s(\xt) \equiv s_a \bmod m$ in the case {\upshape{(iii)}}.
  \end{itemize}
\end{lemma}
 \begin{proof}
