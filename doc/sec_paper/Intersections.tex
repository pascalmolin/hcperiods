...
\begin{document}
  ...
  \begin{thm}[Intersection numbers]\label{thm:intsec_numb}
      ...
      \begin{equation}
	    s(x) = \frac{1}{2\pi}\left( \rho + m \cdot \arg \left( \frac{C_{c,d} \ytcd(x)}{C_{a,b}\ytab(x)} \right) \right) + \frac{1}{2}\delta_{b=c} \mod m
      \end{equation}
     and
      \begin{equation}
          %\rho = \arg(d-c) - \arg(b-a).
...
%  \todo This is wrong. Notice $\ytab(a) = \prod_{x\neq a,b}(\frac{2x-2a}{b-a})^{\frac1m}$, and
